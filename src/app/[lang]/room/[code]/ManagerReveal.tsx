...
export default function ManagerReveal({ room, currentUser, users: initialUsers, lang, dict }: any) {
  ...
  const isAssigned = !!me.manager && !!opponent?.manager
      ...
      {!isAssigned ? (
        <Card className="w-full max-w-lg border-none shadow-2xl bg-white/90 backdrop-blur-md">
          <CardHeader className="text-center pb-6 border-b border-slate-100">
            <CardTitle className="text-3xl font-black text-slate-800">{dict.assignManagers}</CardTitle>
            <p className="text-slate-500 font-medium">{dict.geminiAssign}</p>
          </CardHeader>
          <CardContent className="p-8 text-center">
            {currentUser.role === 'admin' ? (
              ...
              >
                ...
                {isPendingAssign ? dict.revealing : dict.revealManagers}
              </Button>
            ) : (
              <div className="flex flex-col items-center space-y-4">
                <Loader2 className="w-10 h-10 text-blue-500 animate-spin" />
                <p className="text-slate-500 font-medium">{dict.waitingHostAssign}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <>
          <div className="text-center mb-4">
            <h1 className="text-5xl font-black text-slate-800 tracking-tighter mb-2">{dict.managerReveal}</h1>
            <p className="text-slate-500 text-lg font-medium">{dict.squadMastermind}</p>
          </div>
          ...
          <div className="w-full flex flex-col md:flex-row gap-8 justify-center">
            ...
            <div className="flex flex-col items-center space-y-4 animate-in slide-in-from-bottom-8 duration-700 delay-100">
              ...
              <div className="flex items-center text-emerald-600 font-black text-xl bg-emerald-50 px-4 py-2 rounded-full shadow-sm">
                <Coins className={`w-5 h-5 ${lang === 'ar' ? 'ml-1' : 'mr-1'}`} /> {me.budgetLeft}M {dict.left}
              </div>
              ...
              <Card className="w-72 h-96 border-4 border-slate-200 shadow-2xl bg-gradient-to-br from-white to-slate-50 overflow-hidden relative group transform transition-transform hover:scale-105">
                ...
                <CardContent className="p-6 h-full flex flex-col justify-between items-center text-center">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-bold text-blue-500 uppercase tracking-widest">{JSON.parse(me.manager).tier} {dict.tier}</p>
                    <h3 className="text-3xl font-black text-slate-800 leading-tight">{JSON.parse(me.manager).name}</h3>
                  </div>
                  ...
                  <div className="w-full bg-slate-100 p-4 rounded-xl border border-slate-200">
                    ...
                    <p className="font-semibold text-slate-700 leading-snug">{JSON.parse(me.manager).tacticalStyle}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="flex flex-col items-center space-y-4 animate-in slide-in-from-bottom-8 duration-700 delay-300">
              ...
              <Card className="w-72 h-96 border-4 border-slate-200 shadow-2xl bg-gradient-to-br from-white to-slate-50 overflow-hidden relative group transform transition-transform hover:scale-105">
                ...
                <CardContent className="p-6 h-full flex flex-col justify-between items-center text-center">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-bold text-rose-500 uppercase tracking-widest">{JSON.parse(opponent?.manager || '{}').tier} {dict.tier}</p>
                    <h3 className="text-3xl font-black text-slate-800 leading-tight">{JSON.parse(opponent?.manager || '{}').name}</h3>
                  </div>

                  <div className="w-full bg-slate-100 p-4 rounded-xl border border-slate-200">
                    <p className="text-xs font-bold text-slate-400 uppercase mb-1">{dict.tacticalStyle}</p>
                    <p className="font-semibold text-slate-700 leading-snug">{JSON.parse(opponent?.manager || '{}').tacticalStyle}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          {currentUser.role === 'admin' ? (
             <Button 
              ...
              >
                ...
                {isPendingSimulate ? dict.simulating : dict.simulateFinalMatch}
              </Button>
          ) : (
            <div className="mt-12 text-center">
              <Loader2 className="w-8 h-8 text-indigo-500 animate-spin mx-auto mb-2" />
              <p className="text-slate-500 font-bold">{dict.waitingHostSimulate}</p>
            </div>
          )}
        </>
      )}
